...
import com.example.mockodsvue.auth.service.RoleService;
...
public class RoleController {
    ...
    /**
     * 移除使用者的角色
     * DELETE /api/roles/assign/{userCode}/{branchCode}/{roleCode}
     */
    @DeleteMapping("/assign/{userCode}/{branchCode}/{roleCode}")
    public ResponseEntity<Void> removeRole(@PathVariable String userCode,
                                           @PathVariable String branchCode,
                                           @PathVariable String roleCode) {
        roleService.removeRoleFromUser(userCode, branchCode, roleCode);
        return ResponseEntity.noContent().build();
    }
}
