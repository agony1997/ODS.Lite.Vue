...
public class UserDetailsServiceImpl implements UserDetailsService {
     ...
     * @param userCode 使用者編號（當作 username）
     */
    @Override
    public UserDetails loadUserByUsername(String userCode) throws UsernameNotFoundException {
        // 1. 從資料庫查詢使用者
        AuthUser authUser = authUserRepository.findByUserCode(userCode)
                .orElseThrow(() -> new UsernameNotFoundException("找不到使用者: " + userCode));

        // 2. 查詢使用者的角色
        List<AuthUserBranchRole> userRoles = authUserBranchRoleRepository.findByUserCode(userCode);
